$(document).ready(function() {
	...
	var subscription = ['5~CCCAGG~BTC~USD', '5~CCCAGG~ETH~USD','5~CCCAGG~XRP~USD', '5~CCCAGG~BCH~USD','5~CCCAGG~XLM~USD', '5~CCCAGG~LTC~USD'];
	...
	var displayData = function(current, from, tsym, fsym) {
		...
		if (current['PRICE'] > current['OPEN24HOUR']) {
			$('#CHANGE24HOUR_' + from).removeClass();
			$('#CHANGE24HOUR_' + from).addClass("up");
		}
		else if (current['PRICE'] < current['OPEN24HOUR']) {
			$('#CHANGE24HOUR_' + from).removeClass();
			$('#CHANGE24HOUR_' + from).addClass("down");
		}
	};
});